...
export async function savePromotionRulesAction(
    ...
    try {
      ...
      // Only school admin can save promotion rules
...
export async function computeTermResultsAction(
   ...
   // STEP 3: Check role permission - school admin, class teacher, or subject teacher can compute term results
   ...
   if (term.academic_years.school_id !== profile.school_id) {
...
export async function computeYearResultsAction(
   ...
   // STEP 3: Check role permission - school admin, class teacher, or subject teacher can compute year results
   if (!['school_admin', 'class_teacher', 'subject_teacher'].includes(profile.role)) {
     return { success: false, message: 'You do not have permission for this action.' }
   }
...
// STEP 5: Always scope queries to school_id
if (year.school_id !== profile.school_id) {
...
export async function evaluatePromotionAction(
   ...
   // STEP 3: Check role permission - school admin, class teacher, or subject teacher can evaluate promotions
   if (!['school_admin', 'class_teacher', 'subject_teacher'].includes(profile.role)) {
     return { success: false, message: 'You do not have permission for this action.' }
   }
...
export async function executeBulkPromotionAction(
    ...
    // STEP 3: Check role permission - only school_admin can execute bulk promotions
    if (profile.role !== 'school_admin') {
      return { success: false, message: 'You do not have permission for this action.' }
    }
    
    try {
      // STEP 4: For these operations, we need to query/modify data that might be protected by RLS
      // Since we're already authenticated and authorized as school_admin, we can use the regular client
      // and rely on RLS policies combined with our explicit school_id checks for security
      ...
      // Verify the school belongs to the user's school (defense in depth)
      if (school.id !== profile.school_id) {
      ...
      for (const decision of decisions) {
        try {
          ...
          if (decision.outcome === "promoted" && decision.nextClassId && nextYear) {
            ...
            const { data: term } = await supabase.from("terms").select("id").eq("academic_year_id", nextYear.id).eq("status", "upcoming").limit(1).single();
            if (term) {
            ...
            await supabase.from("students")
              .update({ status: "Graduated" as const })
              .eq("id", decision.studentId);
        
            await supabase.from("graduation_records").insert({
              school_id: school.id,
              student_id: decision.studentId,
              academic_year_id: academicYearId,
          ...
          await supabase.from("promotion_audits").insert({
            school_id: school.id,
            academic_year_id: academicYearId,
            student_id: decision.studentId,
      ...
      const sendNotifications = async () => {
        for (const decision of decisions) {
          ...
          const { data: nextClass } = decision.outcome === "promoted"
            ? await supabase.from("classes").select("name").eq("id", decision.nextClassId).single()
            : { data: null };
      ...
      if (nextYear) {
        await supabase.from("academic_years").update({ is_current: false }).eq("id", academicYearId);
        await supabase.from("academic_years").update({ is_current: true }).eq("id", nextYear.id);
      }
      ...
      console.error('Error in executeBulkPromotionAction:', err)
      return { success: false, message: 'An unexpected error occurred' };
    }
  }
