...
export async function savePromotionRulesAction(rules: z.infer<typeof promotionRuleSchema>[]): Promise<ActionResponse> {
  try {
    ...
    for (const rule of rules) {
      const { error } = await auth.supabase.from("promotion_rules").upsert({ school_id: auth.profile.school_id, class_id: rule.classId || null, min_avg_score: rule.minAvgScore, min_subject_score: rule.minSubjectScore, require_all_subjects: rule.requireAllSubjects, updated_at: new Date().toISOString() });
...
export async function computeTermResultsAction(termId: string): Promise<ActionResponse<{ computed: number }>> {
  try {
    ...
    const { data: assessments } = await admin.from("assessments").select("id, class_id, subject_id, assessment_type_id, max_score, assessment_types(weight)").eq("term_id", termId).eq("is_published", true);
    ...
    const { data: enrollments } = await admin.from("student_class_history").select("student_id, class_id").eq("academic_year_id", term.academic_year_id).eq("is_current", true);
    ...
    for (const studentId of [...new Set((grades || []).map((g: any) => g.student_id))]) {
      ...
      for (const subjectId of subjectIds) {
        const items = studentGrades.map((g: any) => {
          ...
          return { score: Number(g.score), maxScore: Number(a.max_score), weight: Number(a.assessment_types?.weight ?? 0), group: String(a.assessment_type_id) };
        }).filter(Boolean) as Array<{ score: number; maxScore: number; weight: number; group: string }>;
      ...
      const { error } = await admin.from("term_results").upsert({ school_id: auth.profile.school_id, student_id: studentId, term_id: termId, academic_year_id: term.academic_year_id, class_id: classId, total_score: avgScore, max_score: 100, avg_score: avgScore, grade_letter: getGradeLetter(avgScore), passed: avgScore >= 50, subject_count: subjectResults.length, subjects_passed: passedSubjects, computed_at: new Date().toISOString() }, { onConflict: "student_id,term_id" });
      if (error) return { success: false, message: error.message };
      computed++;
    }
...
export async function computeYearResultsAction(academicYearId: string): Promise<ActionResponse<{ computed: number }>> {
  try {
    ...
    for (const studentId of [...new Set(termResults.map((r: any) => r.student_id))]) {
      ...
      const avg = results.reduce((s: number, r: any) => s + Number(r.avg_score || 0), 0) / results.length;
      const byTerm = (n: number) => { const id = terms?.find((t: any) => t.term_number === n)?.id; return results.find((r: any) => r.term_id === id)?.avg_score || 0; };
      const { error } = await admin.from("year_results").upsert({ school_id: auth.profile.school_id, student_id: studentId, academic_year_id: academicYearId, class_id: results[0].class_id, term1_avg: byTerm(1), term2_avg: byTerm(2), term3_avg: byTerm(3), yearly_avg: Math.round(avg * 100) / 100, max_score: 100, grade_letter: getGradeLetter(avg), passed: avg >= 50, outcome: "pending", computed_at: new Date().toISOString() }, { onConflict: "student_id,academic_year_id" });
      if (error) return { success: false, message: error.message };
      computed++;
    }
...
export async function evaluatePromotionAction(academicYearId: string): Promise<ActionResponse<{ evaluations: any[]; summary: any }>> {
  try {
    ...
    const latestTerm = (await admin.from("terms").select("id").eq("academic_year_id", academicYearId).eq("school_id", auth.profile.school_id).order("term_number", { ascending: false }).limit(1).single()).data;
    const { data: assignments } = latestTerm ? await admin.from("fee_assignments").select("id").eq("term_id", latestTerm.id) : { data: [] } as any;
    ...
    const evaluations = (students || []).flatMap((student: any) => {
      const r = resultMap.get(student.id); const current = r ? classMap.get(r.class_id) : null;
      if (!r || !current) return [];
      ...
      const balance = balanceMap[student.id] || 0; const next = (classes || []).find((c: any) => c.level === Number(current.level) + 1);
      ...
      return [{ studentId: student.id, studentName: student.full_name, classId: current.id, className: current.name, classLevel: current.level, yearlyAvg: Number(r.yearly_avg || 0), gradeLetter: r.grade_letter || getGradeLetter(Number(r.yearly_avg || 0)), position: 0, passed, hasBalance: balance > 0, balance, nextClassId: next?.id || null, nextClassName: next?.name || null, outcome, blockedReason: balance > 0 ? "Outstanding fees" : passed ? (outcome === "graduated" ? "Graduation recommended" : "Eligible for promotion") : `Below ${rule.min_avg_score}% promotion threshold`, eligible: balance <= 0 && passed }];
    });
    const grouped: Record<string, any[]> = {}; evaluations.forEach((e: any) => (grouped[e.classId] ||= []).push(e));
    Object.values(grouped).forEach((arr: any[]) => arr.sort((a, b) => b.yearlyAvg - a.yearlyAvg).forEach((e, i) => e.position = i + 1));
    const summary = { total: evaluations.length, eligible: evaluations.filter((e: any) => e.eligible).length, blockedFees: evaluations.filter((e: any) => e.hasBalance).length, failed: evaluations.filter((e: any) => !e.passed).length, graduated: evaluations.filter((e: any) => e.outcome === "graduated").length };
...
export async function executeBulkPromotionAction(academicYearId: string, decisions: Array<{ studentId: string; currentClassId: string; outcome: "promoted" | "repeated" | "graduated" | "withdrawn"; nextClassId?: string }>): Promise<ActionResponse> {
  try {
    ...
    for (const decision of decisions) {
      ...
      if (toClassId && decision.outcome !== "graduated") await admin.from("student_class_history").insert({ student_id: decision.studentId, class_id: toClassId, academic_year_id: academicYearId, outcome: "active", is_current: true });
...
export async function getYearEndDataAction(academicYearId: string): Promise<{ students: any[]; classes: any[]; terms: any[]; rules: any[]; yearResults: any[]; audits: any[] }> {
  try {
    ...
    const [{ data: students }, { data: classes }, { data: terms }, { data: rules }, { data: yearResults }, { data: audits }] = await Promise.all([
      admin.from("student_class_history").select("student_id, class_id, students!inner(full_name, school_id), classes(name, level)").eq("academic_year_id", academicYearId).eq("is_current", true).eq("students.school_id", auth.profile.school_id),
